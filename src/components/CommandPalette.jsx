...
const CommandPalette = ({ isDarkMode, toggleTheme }) => {
  ...
  const paletteStyle = isDarkMode 
    ? "bg-[#2D3748] text-slate-200 border-slate-600" 
    : "bg-[#FDFBF7] text-slate-800 border-gray-200";
    ...
    >
      <div className={`w-full max-w-lg rounded-xl shadow-2xl border overflow-hidden ${paletteStyle}`}>
        ...
        <Command.Input 
          ...
          className={`w-full px-4 py-4 text-base bg-transparent outline-none border-b ${isDarkMode ? 'border-slate-600 placeholder:text-slate-500' : 'border-gray-200 placeholder:text-gray-400'}`}
        />
        ...
        <Command.List className="max-h-[300px] overflow-y-auto p-2 scrollbar-hide">
          ...
          <Command.Group heading="General" className="px-2 py-1.5 text-xs font-medium opacity-50 uppercase tracking-widest">
            ...
            <Command.Item onSelect={() => { toggleTheme(); setOpen(false); }} className={itemStyle}>
              {isDarkMode ? <Sun size={14} /> : <Moon size={14} />}
              <span>Switch to {isDarkMode ? 'Light' : 'Dark'} Mode</span>
            </Command.Item>

            <Command.Item onSelect={handleDownloadResume} className={itemStyle}>
              <FileText size={14} />
              <span>Download Resume</span>
            </Command.Item>

            <Command.Item onSelect={handleCopyEmail} className={itemStyle}>
              <Mail size={14} />
          ...
          <Command.Group heading="Socials" className="px-2 py-1.5 mt-2 text-xs font-medium opacity-50 uppercase tracking-widest">
            <Command.Item onSelect={() => openLink(SOCIAL_LINKS.github)} className={itemStyle}>
              <Github size={14} />
              <span>GitHub</span>
              <ExternalLink size={10} className="ml-auto opacity-50"/>
            </Command.Item>
            <Command.Item onSelect={() => openLink(SOCIAL_LINKS.linkedin)} className={itemStyle}>
              <Linkedin size={14} />
              <span>LinkedIn</span>
              <ExternalLink size={10} className="ml-auto opacity-50"/>
            </Command.Item>
        ...
        {/* Footer Hint */}
        <div className={`px-4 py-2 text-[10px] border-t flex justify-end opacity-50 ${isDarkMode ? 'border-slate-600' : 'border-gray-200'}`}>
           Use arrow keys <span className="mx-1 border rounded px-1">↑</span><span className="border rounded px-1">↓</span> to navigate
        </div>
