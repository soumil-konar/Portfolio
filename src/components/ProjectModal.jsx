        ...
        <motion.div
          ...
          className={`relative w-full max-w-3xl rounded-2xl border shadow-2xl overflow-hidden z-10 flex flex-col max-h-[85vh] backdrop-blur-2xl ${
            isDarkMode 
              ...
              : 'bg-white/98 border-indigo-100 text-slate-800'
          }`}
        >
          {/* Top Bar / Header */}
          <div className={`px-6 py-5 border-b flex items-center justify-between shrink-0 ${
            isDarkMode ? 'border-slate-800 bg-slate-950/40' : 'border-indigo-50 bg-indigo-50/30'
          }`}>
            <div className="flex items-center space-x-3.5">
              <div className={`p-2.5 rounded-xl ${theme.accent} backdrop-blur-sm ${
                isDarkMode ? 'bg-indigo-500/20' : 'bg-indigo-100/70'
              }`}>
                {project.icon}
              </div>
              <div>
                <div className="flex items-center space-x-2">
                  <h3 className="font-bold text-lg md:text-xl tracking-tight">{project.title}</h3>
                </div>
                <div className="flex items-center space-x-2 mt-0.5">
                  {project.github ? (
                    ...
                    <span className="text-[11px] font-mono opacity-60 flex items-center space-x-1">
                      <Lock size={10} />
            ...
            <div className="flex items-center space-x-2">
              {project.github && (
                <a
                  ...
                  className={`p-2 px-3 rounded-lg border text-xs font-mono flex items-center space-x-1.5 transition-all shadow-sm ${
                    isDarkMode
          ...
          <div className={`px-6 pt-3 flex space-x-6 border-b text-xs font-mono shrink-0 ${
            isDarkMode ? 'border-slate-800 bg-slate-950/20' : 'border-indigo-50 bg-indigo-50/10'
          }`}>
            {[
              ...
              <button
                ...
                className={`flex items-center space-x-1.5 pb-2.5 border-b-2 transition-all cursor-pointer ${
                  activeTab === tab.id
          ...
          <div className="p-6 overflow-y-auto space-y-6 flex-1">
            ...
            {project.problem && (
              <div className={`p-4 rounded-xl border leading-relaxed text-xs md:text-sm font-sans ${
                ...
                <span className="font-bold uppercase tracking-wider text-[10px] font-mono opacity-60 block mb-1 text-indigo-400">
            ...
            {activeTab === 'architecture' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                <h4 className="font-mono text-xs uppercase tracking-wider opacity-60 flex items-center space-x-1.5">
                  <Workflow size={13} />
                ...
                {project.architecture && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {project.architecture.map((node, i) => (
                      ...
                      >
                        <div className="flex items-center space-x-2 text-indigo-400 font-mono font-bold text-xs mb-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-indigo-500" />
                          <span>{node.step}</span>
                        </div>
                        <p className="text-xs opacity-75 font-sans leading-relaxed">{node.detail}</p>
                      </div>
                    ))}
                  </div>
                )}
                    ...
                    {project.highlights.map((item, i) => (
                      <div
                        ...
                        className={`flex items-start space-x-3 p-3 rounded-xl border ${
                          isDarkMode ? 'bg-slate-950/40 border-slate-800' : 'bg-white border-slate-100 shadow-sm'
                        }`}
                      >
                        <ShieldCheck size={16} className="text-emerald-400 shrink-0 mt-0.5" />
                        <span className="text-xs md:text-sm font-sans leading-relaxed opacity-85">{item}</span>
                      </div>
                    ))}
                  </div>
                )}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {project.metrics.map((metric, i) => (
                      ...
                      >
                        ...
                        <span className="font-mono font-bold text-sm md:text-base text-indigo-400">
                          {metric.val}
                        </span>
                      </div>
                    ))}
          ...
          <div className={`px-6 py-4 border-t flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0 ${
            isDarkMode ? 'border-slate-800 bg-slate-950/60' : 'border-indigo-50 bg-indigo-50/20'
          }`}>
            <div className="flex flex-wrap gap-1.5 items-center">
              <span className="text-[10px] font-mono opacity-50 uppercase mr-1">Stack:</span>
            ...
            <div className="text-[10px] font-mono opacity-40 self-end sm:self-auto">
              Press <kbd className="px-1 py-0.5 border rounded">ESC</kbd> to exit
            </div>
          </div>
        </motion.div>
      </div>
